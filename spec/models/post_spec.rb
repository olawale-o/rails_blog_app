...
RSpec.describe Post, type: :model do
  ...
  describe 'most_recent_comments limit 5' do
    ...
    it 'when total comments for a post is greater than 5 limit comments to 5' do
      ...
      comment7 = Comment.new(user_id: 1, post_id: subject.id, text: 'Comment 7')
      subject.comments << comment7
      comment8 = Comment.new(user_id: 1, post_id: subject.id, text: 'Comment 8')
      subject.comments << comment8
      comment9 = Comment.new(user_id: 1, post_id: subject.id, text: 'Comment 9')
      subject.comments << comment9
      expect(subject.most_recent_comments.size).to be <= 5
    end
  end
end
